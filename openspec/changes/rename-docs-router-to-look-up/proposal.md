...
- SKILL.md 新增「Explicit invocation」段落：定義 $ARGUMENTS 依參數形態的確定性路由（三形態 + fallback chain）— 語言名走 runtime introspection 並錨定本機版本，引擎無 adapter 或無版本來源時（R 現況）誠實回報 not-resolved 改答 web-latest；套件名（可帶 name@version，last-@ 切分）走 resolve_source 再 fetch_docs，ecosystem 由 calling agent 具名（npm/pypi 自動偵測），version pin 僅 npm/pypi 生效、其餘 ecosystem 明示 pin 未套用；URL 預設走 docs-site resolution（resolve_source docs_url），顯式 API schema path（/graphql、/openapi.json 等）或 docs-site 落空才走 OpenAPI/GraphQL introspection；CLI introspection 是 Package 形態的 resolution-time fallback；無參數則載入既有路由指引（行為同 implicit）。
