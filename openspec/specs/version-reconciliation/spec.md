...
The system SHALL separate reconciliation into a **detect** phase and an **offer** phase.
...
In the **offer** phase, the system SHALL offer an upgrade only when the answer is version-sensitive OR an actual version skew or error is observed, and SHALL NOT offer upgrades for queries where the version gap is irrelevant.
...
#### Scenario: Offer an upgrade only when relevant

- **WHEN** the local version is behind web-latest AND the query is version-sensitive or an actual skew/error is observed
- **THEN** the system offers an upgrade
- **AND** for an unrelated query where the version gap does not affect the answer, the system does not offer an upgrade
